...
to fire-spread
    ask patches[
      ...
      if pcolor = brown + 4 and any? neighbors with [pcolor = red] [
        if random 100 < 5[ ;%5 pravdepodobnost sireni ohne
          set pcolor red
        ]
      ]
...
to blow-up
  ...
  ask patches[
    if ((firex - pxcor)) ^ 2 + (firey - pycor) ^ 2 <= 10 [ ;kruh, vybuch kruhem po polomeru 10, (x-a)^2 + (y-b)^2 <= r^2, nebude to presny kruh, pracujeme s integers, pokud vetsi, tak <=100
      set pcolor red
      ]
    ]
...
to setup-patches
  ...
  ask patches[
    ...
    if (pxcor > (doorx - 2 )) and (pxcor < (doorx + 2)) and
     (pycor > (doory - 2 )) and (pycor < (doory + 2))[
     set pcolor green
     ]
  ]
